...
def load_mnist_rates(in_path, class_idx, min_noise=0, max_noise=0, mean_rate=None, suffix=None):
    ...
    for fname in fnames:
        ...
        if cls == class_idx:
            ...
            on_rates = np.memmap(filename, dtype='uint16', mode='w+', shape=(n_smpls, height, width))
            on_rates[:] = np.round(np.random.uniform(min_noise, max_noise, size=(n_smpls, height, width))).astype('uint16')
            for i in xrange(len(data[ON][IDX])):
                for idx in data[ON][IDX][i]:
                    ...
                    if mean_rate is None:
                        ...
                        on_rates[i, r, c] += round(new_rate / len(data[ON][IDX][i]))
            on_rates[on_rates < 0] = 0.
            np.random.seed()
            filename = os.path.join(tmp_dir, 'off_rates.dat')
            off_rates = np.memmap(filename, dtype='uint16', mode='w+', shape=(n_smpls, height, width))
            off_rates[:] = np.round(np.random.uniform(min_noise, max_noise, size=(n_smpls, height, width))).astype('uint16')
            for i in xrange(len(data[OFF][IDX])):
                for idx in data[OFF][IDX][i]:
                    ...
                    if mean_rate is None:
                        ...
                        off_rates[i, r, c] += round(new_rate / len(data[OFF][IDX][i]))
